...
def test_data_transformation(df):
    ...
    try:
        ...
        df_transformed["quantity_in_stock"] = df_transformed[
            "quantity_in_stock"
        ].fillna(0)
        ...
        df_transformed["stock_status"] = df_transformed["quantity_in_stock"].apply(
            lambda x: (
                "Out of Stock" if x == 0 else "Low Stock" if x < 10 else "In Stock"
            )
        )

        print(f"✅ Data transformed successfully!")
        print(
            f"   Added price_category column: {df_transformed['price_category'].value_counts().to_dict()}"
        )
        print(
            f"   Added stock_status column: {df_transformed['stock_status'].value_counts().to_dict()}"
        )
...
def test_data_loading(df, engine):
    ...
    try:
        ...
        # Load to database
        test_df.to_sql(
            "products_test", engine, if_exists="replace", index=False, method="multi"
        )
    ...
    try:
        ...
        with engine.connect() as connection:
            ...
            result = connection.execute(
                text("SELECT COUNT(*) FROM products_test WHERE product_id IS NULL")
            )
            ...
            result = connection.execute(
                text("SELECT COUNT(*) FROM products_test WHERE price < 0")
            )
            ...
            result = connection.execute(
                text(
                    "SELECT COUNT(*) FROM products_test WHERE rating < 1.0 OR rating > 5.0"
                )
            )
            ...
            result = connection.execute(
                text("SELECT COUNT(DISTINCT price_category) FROM products_test")
            )
