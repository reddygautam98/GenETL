...
try:
    ...
    engine = create_engine(
        "postgresql://genetl:genetl_pass@localhost:5450/genetl_warehouse"
    )
    ...
    # Data Quality Test
    quality = pd.read_sql(
        "SELECT COUNT(*) as count FROM logs.data_quality_checks WHERE status = 'passed'",
        engine,
    )
    ...
    pipeline = pd.read_sql(
        "SELECT COALESCE(SUM(records_processed), 0) as total FROM logs.etl_pipeline_runs WHERE status = 'success'",
        engine,
    )
