...
def transform_products_data(**context):
    ...
    try:
        ...
        logging.info(
            f"Transformation complete: {final_count} records ({removed_count} removed)"
        )
...
def validate_data_quality(**context):
    ...
    try:
        ...
        validation_results.append(
            {"test": "null_product_ids", "passed": null_ids == 0, "value": null_ids}
        )
...
def generate_summary_report(**context):
    ...
    try:
        ...
        logging.info(
            f"Records Extracted: {extract_result.get('records_extracted', 'N/A')}"
        )
        logging.info(f"Records Loaded: {load_result.get('records_loaded', 'N/A')}")
        logging.info(
            f"Validation Status: {'PASS' if validation_result.get('validation_success') else 'FAIL'}"
        )
...
extract_task = PythonOperator(
    task_id="extract_data", python_callable=extract_products_data, dag=dag
)

transform_task = PythonOperator(
    task_id="transform_data", python_callable=transform_products_data, dag=dag
)

load_task = PythonOperator(
    task_id="load_data", python_callable=load_products_data, dag=dag
)

validate_task = PythonOperator(
    task_id="validate_data", python_callable=validate_data_quality, dag=dag
)

report_task = PythonOperator(
    task_id="generate_report", python_callable=generate_summary_report, dag=dag
)
