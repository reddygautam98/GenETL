...
REDIS_CONFIG = {"host": "localhost", "port": 6390}
...
def test_database_connection():
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            print_status(
                f"PostgreSQL connected successfully: {version.split(',')[0]}", "SUCCESS"
            )
            return engine
...
def test_redis_connection():
    ...
    try:
        ...
        info = r.info()
        print_status(
            f"Redis connected successfully: Version {info['redis_version']}", "SUCCESS"
        )
        return r
    except Exception as e:
...
def check_database_schemas(engine):
    ...
    try:
        ...
        if required_schemas.issubset(existing_schemas):
            print_status(
                f"All required schemas present: {', '.join(sorted(existing_schemas))}",
                "SUCCESS",
            )
            return True
        else:
    ...
    try:
        ...
        if len(tables) > 0:
            ...
            for _, row in tables.iterrows():
                print(
                    f"   📋 {row['schemaname']}.{row['tablename']}: {row['record_count']:,} records (stats)"
                )
            ...
            for _, row in actual_counts.iterrows():
                print(
                    f"   📊 {row['table_name']}: {row['actual_count']:,} records (actual)"
                )
...
def check_data_quality_logs(engine):
    ...
    try:
        ...
        if len(quality_checks) > 0:
            ...
            print_status(
                f"Data quality checks: {len(passed_checks)}/{len(quality_checks)} passed",
            ...
            for _, row in quality_checks.iterrows():
                ...
                print(
                    f"   {status_icon} {row['check_name']}: {row['status']} (value: {row['actual_value']})"
                )
            return True
        else:
...
def check_etl_pipeline_logs(engine):
    ...
    try:
        ...
        if len(pipeline_logs) > 0:
            ...
            print_status(
                f"Pipeline runs: {len(successful_runs)}/{len(pipeline_logs)} successful",
            ...
            for _, row in pipeline_logs.iterrows():
                ...
                print(
                    f"   {status_icon} {row['pipeline_name']}: {row['records_processed']} processed, {row['records_success']} successful"
                )
            return True
        else:
...
def test_sample_etl_operation(engine):
    ...
    try:
        ...
        if len(result) > 0 and result.iloc[0]["total_records"] > 0:
            row = result.iloc[0]
            print_status(
                f"Sample query successful: {row['total_records']:,} products, {row['categories']} categories, avg price ${row['avg_price']}",
                "SUCCESS",
            )
            return True
        else:
...
def run_comprehensive_health_check():
    ...
    overall_status = (
        ...
        else "WARNING" if passed_checks > total_checks // 2 else "ERROR"
    )
    print_status(
        f"Overall Health: {passed_checks}/{total_checks} checks passed", overall_status
    )
    ...
    if overall_status == "SUCCESS":
        ...
        print_status(
            "⚠️ ETL Process has some issues but core functionality works", "WARNING"
        )
    else:
