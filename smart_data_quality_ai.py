...
class SmartDataQualityAI:
    ...
    def _initialize_ai_rules(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "rating_rules": {"min_value": 1.0, "max_value": 5.0, "decimal_places": 1},
            "text_rules": {
                "min_length": 2,
                "max_length": 255,
            ...
            "stock_rules": {"min_value": 0, "max_value": 10000, "negative_alert": True},
        }

    def load_data_for_analysis(
        self, table_name: str = "warehouse.products"
    ) -> pd.DataFrame:
        ...
        try:
            ...
            logger.info(
                f"Loaded {len(df)} records from {table_name} for quality analysis"
            )
            return df
    ...
    def ai_detect_outliers(
        ...
        try:
            ...
            if method == "iqr":
                ...
                threshold = self.quality_rules.get("price_rules", {}).get(
                    "outlier_threshold", 3
                )
            ...
            for column in df.columns:
                ...
                if null_percentage > 0:
                    severity = (
                        ...
                        else "HIGH" if null_percentage > 5 else "MEDIUM"
                    )
    ...
    def ai_consistency_check(self, df: pd.DataFrame) -> List[QualityIssue]:
        ...
        try:
            ...
            if "price" in df.columns:
                price_outliers, confidence = self.ai_detect_outliers(
                    df, "price", "modified_zscore"
                )
            ...
            if "rating" in df.columns:
                invalid_ratings = df[(df["rating"] < 1.0) | (df["rating"] > 5.0)].shape[
                    0
                ]
    ...
    def ai_format_validation(self, df: pd.DataFrame) -> List[QualityIssue]:
        ...
        try:
            ...
            if "product_id" in df.columns:
                ...
                if (
                    pattern_analysis
                    and pattern_analysis.get("compliance_rate", 1.0) < 0.9
                ):
            ...
            for column in email_columns:
                if column in df.columns:
                    ...
                    invalid_emails = df[
                        ~df[column].astype(str).str.match(email_pattern, na=False)
                    ].shape[0]
    ...
    def generate_data_profile(self, df: pd.DataFrame) -> List[DataProfile]:
        ...
        try:
            for column in df.columns:
                ...
                uniqueness_score = (
                    (unique_count / len(df)) * 100 if "id" in column.lower() else 100
                )
                ...
                if completeness_score < 95:
                    recommendations.append(
                        f"Improve data completeness (currently {completeness_score:.1f}%)"
                    )
                if consistency_score < 90:
                    recommendations.append(
                        f"Address data consistency issues ({outlier_count} outliers detected)"
                    )
                if format_score < 85:
                    recommendations.append(
                        f"Standardize data formats ({pattern_violations} format violations)"
                    )
    ...
    def run_comprehensive_quality_assessment(self) -> Dict[str, Any]:
        ...
        if data_profiles:
            overall_quality_score = np.mean(
                [profile.quality_score for profile in data_profiles]
            )
        else:
        ...
        logger.info(
            f"✅ Quality assessment complete! Overall score: {overall_quality_score:.1f}%"
        )
        return results
...
def main():
    ...
    print(
        f"Overall Quality Score: {results['overall_quality_score']}% ({results['quality_grade']})"
    )
    ...
    for i, issue in enumerate(results["quality_issues"], 1):
        ...
        print(
            f"   Column: {issue['column']} | Affected: {issue['affected_count']} records"
        )
